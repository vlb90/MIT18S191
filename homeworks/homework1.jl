...
student = (name = "Vinícius Ludwig Barbosa", kerberos_id = "vlb")
...
Submission by: **_$(student.name)_** ($(student.kerberos_id)@bth.se)
"""
...
# ╔═╡ ac019c52-f504-11ea-2948-4bd6306a833d
begin
...
# ╔═╡ 738631da-f2ed-11ea-0955-e9e674f6cc63
typeof([[1,2], [3,4]])

# ╔═╡ c279ece0-f2ee-11ea-1c8f-dfc6748f673b
[[1,2], [3,4]]
...
function matrix_to_vecvec(matrix)
	...
	return [matrix[:,i] for i=1:size(matrix,2)]
...
function mean_colors(image)
	...
	return (sred, sgreen, sblue)./pixels
...
# ╔═╡ 7f35b182-f364-11ea-0b82-ff8f57cc945b
function myclamp(x)
...
# ╔═╡ 436f4cdc-f37d-11ea-0388-0b95fc8b47ff
#test = rand(-2:0.025:2,1)

# ╔═╡ 8d0abc0a-f380-11ea-2a18-e95614208a1d
#typeof(test)

# ╔═╡ a540d3fc-f387-11ea-100c-17cdaf3b9fb7
rand(2:0.025:2)
...
begin
	...
	function noisify(color::AbstractRGB, s)
		# you will write me in a later exercise!
		nr = myclamp(noisify(color.r, rand(-s:0.001:s)))
		ng = myclamp(noisify(color.g, rand(-s:0.001:s)))
		nb = myclamp(noisify(color.b, rand(-s:0.001:s)))
		return RGB(nr,ng,nb)
	end
...
# ╔═╡ beee46f4-f382-11ea-18b7-019cfb0bd805
test_value = noisify(red.r, rand(-2:0.025:2))

# ╔═╡ e64f6b2c-f37a-11ea-1977-ebf0056afb89
cor = myclamp(noisify(red.r, test_value))

# ╔═╡ 9741802a-f383-11ea-1d43-5d1a2c2edc67
typeof(cor)

# ╔═╡ f02f4510-f382-11ea-1c20-b3cc7e1bfa09
RGB(cor[1], 0, 0)
...
The image is unrecognisable with intensity larger than 1 since RGB channels tend to be more clamped.
"""
...
# ╔═╡ e2cf6142-f35e-11ea-30f4-cd845ecd0500
typeof(philip[300, 300])
...
# ╔═╡ 49ed4cbc-f3a8-11ea-19eb-b956e3b32fc8
[extend(example_vector, i) for i in -1:12]

# ╔═╡ 5242d670-f3a8-11ea-154b-55f17186a92d
size(example_vector)
...
function blur_1D(v, l)
	pad = 2*l + 1
	kblur = 1/pad
	v_blur = zeros(size(v,1))
	#v_extended = [extend(v,i) for i in -l:end+l]
	for i in 1:size(v,1)
		v_blur[i] = sum([extend(v,j)*kblur for j in -l+i:i+l])
	end
...
# ╔═╡ b9d86246-f3b2-11ea-0443-354b79f74d71
colored_line(example_vector)

# ╔═╡ d2839ba6-f3a8-11ea-2354-ebb97595e48c
colored_line(blur_1D(example_vector, 1))
...
# ╔═╡ a3101406-f3a5-11ea-2218-4d7706aecaa9
@bind l_box Slider(1:size(v,1), show_value=true)

# ╔═╡ 58bea3a6-f3b4-11ea-2a8e-3f542c6fffa5
2*l_box +1, 1/(2*l_box +1)

# ╔═╡ 91d442aa-f3b3-11ea-0b05-57764741cdba
colored_line(v)
...
# ╔═╡ b0c9b056-f3b7-11ea-1daf-873682d9a4d1
begin
	k = rand(5)
	x = [i for i=1:5]
	k.*x
...
# ╔═╡ fd6c40ea-f3b7-11ea-3ff6-c12529851366
x

# ╔═╡ ef12aef8-f3b7-11ea-193f-576172f9f979
k
...
function convolve_vector(v, k)
	...
	v_blur = zeros(size(v,1))
	for i in 1:size(v,1)
		buffer = [extend(v,j) for j in -l+i:i+l]
		v_blur[i] = sum(buffer.*k)
	end
...
#=	l = (n-1)÷2
	gaus(x) = (1/(sqrt(2pi)))*exp(-0.5x^2)
	return [gaus(sqrt(i^2+j^2)) for i=-l:l, j=-l:l]/gaus(0)
=#

	# 1D
	l = (n-1)÷2
	gaus(x) = (1/(sqrt(2pi)))*exp(-0.5*x^2)
	return [gaus(i) for i=-l:l]./gaus(0)
	#return missing
end

# ╔═╡ aeaf37b4-f3bc-11ea-1e41-69d30cfd34eb
gaussian_kernel(1)
...
# ╔═╡ 8beb8d82-f41a-11ea-0dc2-1746793dedde
md"Original:"

# ╔═╡ 9a29cd1e-f41a-11ea-1b88-e18addaa0593
md" Blurred version (window length **$gaussian_kernel_size_1D**):"

# ╔═╡ 2acb02ee-f41a-11ea-1fed-435657742cb8
test_vector = rand(15)

# ╔═╡ 3835f076-f41a-11ea-2bbe-0ff73a5d51dc
colored_line(test_vector)
...
function extend_mat(M::AbstractMatrix, i, j)
	...
	elseif 	1<=i<=size(M,1) && j<1

		return M[i,1]

	elseif 	1<=i<=size(M,1) && j>size(M,2)

		return M[i,end]

	elseif 	i>size(M,1) && 1<=j<=size(M,2)
...
# ╔═╡ f982b70a-f422-11ea-2c02-8fb5fb2ab604
extend_mat([42 37; 1 0], -1, 3)
...
function convolve_image(M::AbstractMatrix, K::AbstractMatrix)
	...
	for i in 1:size(M,1)
		for j in 1:size(M,2)
			new_M[i,j]= sum([extend_mat(M,v,h) for v=i-l:i+l, h=j-l:j+l].*parent(K))
		end
	end
	return new_M
end

# ╔═╡ 951fdc70-f50d-11ea-3ea7-11d6f0fa7059
...
# ╔═╡ 3b8ef818-f42b-11ea-0b36-25726a25ab0d
philip

# ╔═╡ e7f8b41a-ee25-11ea-287a-e75d33fbd98b
new_phil = convolve_image(philip, K_test)
...
# ╔═╡ b2b78fb4-f436-11ea-0134-a75249e2264a
function gaussian_kernel_2D(stdv)
	# implement a square kernel with σx = σy
	radius = ceil(3*stdv)
	n = 2*radius + 1
	l = (n-1)÷2
	s2 = stdv^2
	gaus(x,y,s2) = (0.5/(pi*s2))*exp(-((x^2+y^2)*0.5)/(s2))
	return [gaus(i,j,s2) for i=-l:l, j=-l:l]#./gaus(0,0)
end

# ╔═╡ aad67fd0-ee15-11ea-00d4-274ec3cda3a3
function with_gaussian_blur(image, sigma)
	blur_image = copy(image)
	#return convolve_image(blur_image, test_kernel(13))
	return convolve_image(blur_image, gaussian_kernel_2D(sigma))
	#return missing
end

# ╔═╡ 106586de-f507-11ea-12ad-6b8d7172cf4f
@bind sigma Slider(1:10, show_value=true)

# ╔═╡ 4b3146d6-f4da-11ea-3776-e9d32b165a82
[philip with_gaussian_blur(philip, sigma) convolve_image(philip, Kernel.gaussian((sigma,sigma)))] # [original my_function pkg]
...
# ╔═╡ c45ad1c6-f5a9-11ea-30bd-95e22fc0355c
function image_channel(pixel, channel)
	new_pixel = copy(pixel)
	if channel == "red"
		return Float64(new_pixel.r)
	elseif channel == "green"
		return Float64(new_pixel.g)
	elseif channel == "blue"
		return Float64(new_pixel.b)
	end
end
...
function with_sobel_edge_detect(image)
	...
	image_red 	= image_channel.(image,"red")
	image_green = image_channel.(image,"green")
	image_blue 	= image_channel.(image,"blue")

	x_mat = [1;2;1].*[1 0 -1]
	y_mat = [1;0;-1].*[1 2 1]

	Gx_r = convolve_image(image_red, x_mat)
	Gx_g = convolve_image(image_green, x_mat)
	Gx_b = convolve_image(image_blue, x_mat)

	Gy_r = convolve_image(image_red, y_mat)
	Gy_g = convolve_image(image_green, y_mat)
	Gy_b = convolve_image(image_blue, y_mat)

	Gt_r = sqrt.(Gx_r.^2 + Gy_r.^2)
	Gt_g = sqrt.(Gx_g.^2 + Gy_g.^2)
	Gt_b = sqrt.(Gx_b.^2 + Gy_b.^2)

	return Gray.(RGB.(Gt_r, Gt_g, Gt_b))
end

# ╔═╡ 9f4b8d12-f5bb-11ea-2c11-618cb878f40b
...
@bind gauss_raw_camera_data camera_input(;max_size=800)

# ╔═╡ 1a0324de-ee19-11ea-1d4d-db37f4136ad3
@bind sobel_raw_camera_data camera_input(;max_size=800)
